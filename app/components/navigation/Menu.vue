...
<template>
  <nav class="sticky top-0 z-50 w-full transition-all bg-white"
    ...
    <div class="container">
      <div
        ...
        <NuxtLink v-if="data?.logo" to="/" class="w-fit" @click="navigationMenuStore.close()">
          <AtomsImage :image="data?.logo" class="object-contain object-left w-full h-12" />
          <span class="sr-only">Site Logo</span>
        </NuxtLink>

        <NavigationHamburger v-if="data?.routes" />

        <div v-if="data?.routes" class="items-center justify-end gap-8 lg:flex"
          :class="navigationMenuStore.state ? 'col-span-2' : 'hidden'">
          <NavigationRoutes v-if="data?.routes" :routes="data?.routes" />

          <AtomsButton v-if="data?.button && data?.button.id" :id="data?.button.id"
