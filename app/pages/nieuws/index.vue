<script setup lang="ts">
import type { Page, GlobalSettings } from "#payload/types";

const { data: globalSettings, error: globalError } = await useGlobal<GlobalSettings>('global-settings');

const isGlobalError = ref(!!globalError.value);
if (isGlobalError.value) console.log(isGlobalError.value)

const blogPageId = globalSettings.value?.blog?.page?.id;

const isPageError = ref(false);
const pageErrorMessage = ref('');
let archiveBlocks: unknown[] = [];

if (!isGlobalError.value && blogPageId) {
    const { data: pageData, error: pageError } = await useCollection<Page>('pages', { id: blogPageId });

    isPageError.value = !!pageError.value || !pageData.value;
    pageErrorMessage.value = pageError.value?.message || 'Failed to load blog page';

    if (!isPageError.value) {
        archiveBlocks = (pageData.value as Page)?.archive?.blocks ?? [];
    }
} else if (!blogPageId) {
    isPageError.value = true;
    pageErrorMessage.value = 'Blog page configuration not found';
}

if (import.meta.server && process.env.NODE_ENV !== 'production') {
    console.log(`[Prerender /nieuws] Global error: ${isGlobalError.value}, Page error: ${isPageError.value}`);
}
</script>

<template>
    <div>
        <template v-for="(block, blockIndex) in archiveBlocks" :key="blockIndex">
            <Component :is="`blocks-${block.blockType}`" v-if="block.id" :data="block" />
        </template>

        <OrganismsArticles />
    </div>
</template>
